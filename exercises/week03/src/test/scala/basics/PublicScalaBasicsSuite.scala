...
class PublicScalaBasicsSuite extends FunSuite {
    ...
    test("[4] The minWhile function should throw an IllegalArgumentException if an empty array is passed") {
        ...
        assertThrows[IllegalArgumentException]{ minWhile(Array()) }
    }
    ...
    test("[10] should count the words") {
        ...
        assert(wordCounter(input) == result)
    }
